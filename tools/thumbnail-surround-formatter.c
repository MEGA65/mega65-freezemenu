...
int palette_lookup(struct tile_set* ts, int r, int g, int b)
{
  ...
  fprintf(stderr, "Matching colour #%02x%02x%02x", r, g, b);
  ...
  for (i = 1; i < ts->colour_count; i++) {
    ...
    // brightness
    int colour_error = 3 * abs(r - ts->colours[i].r) * abs(r - ts->colours[i].r)
    ...
    if (r == ts->colours[i].r && g == ts->colours[i].g && b == ts->colours[i].b) {
      ...
      fprintf(stderr, " -- exactly matches colour 0x%02x\n", i);
      return i;
    }
  ...
  fprintf(stderr, " -- approximately matches colour 0x%02x\n", best_colour);
  return best_colour;
}
...
int main(int argc, char** argv)
{
  ...
  */
  printf("Writing %d screens", screen_count - 1);
  fflush(stdout);
  for (i = 1; i < screen_count; i++) {
    ...
    unsigned int screenram_rows_offset = 64;
    unsigned int colourram_rows_offset = screenram_rows_offset + (2 * screen_list[i]->width) * screen_list[i]->height;
